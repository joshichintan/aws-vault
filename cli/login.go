...
func ConfigureLoginCommand(a *AwsVault) *cobra.Command {
	...
	autoLogoutDefault := false
	if os.Getenv("AWS_VAULT_AUTO_LOGOUT") == "true" {
		autoLogoutDefault = true
	}
	stdoutDefault := false
	...
	cmd.Flags().BoolVarP(&input.AutoLogout, "auto-logout", "a", autoLogoutDefault, "Auto logout when starting a new login")
